...
*The Accuracy for this model is about 74%. This corresponds to the Out of sample error. For this methode, i expected the accuracy to be little higher than 75%*
...
##Model fitting
   modrf <- train(classe ~ ., data = train,method = "rf", metric = "Accuracy", preProcess=c("center", "scale"), trControl=trainControl(method = "cv", number = 5) ##CROSS VARIDATION WITH K=5)
...
*The accuracy for random forest is almost 99.22%. The Out of sample error is quite higher so I pick it as the best algorithm for prediction as compared to the previous on since the accuracy quite good which means it can do well on unknown datasets.* 
